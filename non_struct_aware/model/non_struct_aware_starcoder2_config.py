...
from non_struct_aware.model.non_struct_aware_mcore_gpt_model import NonStructAwareMCoreGPTModel
from non_struct_aware.self_attention.non_struct_aware_layer_spec import non_struct_aware_layer_spec
...
def non_struct_aware_gpt_data_step(dataloader_iter) -> Dict[str, torch.Tensor]:
	from megatron.core import parallel_state
	...
	if parallel_state.is_pipeline_first_stage():
		required_device_keys.update(("code_token_ids", "code_token_rel_pos_ids", "attention_bias"))
...
def non_struct_aware_gpt_forward_step(model, batch) -> torch.Tensor:
	forward_args = {
...
@dataclass
class NonStructAwareStarcoder2Config(Starcoder2Config3B):

	transformer_layer_spec: Union[ModuleSpec, Callable[["GPTConfig"], ModuleSpec]] = non_struct_aware_layer_spec
	forward_step_fn: Callable = non_struct_aware_gpt_forward_step
	data_step_fn: Callable = non_struct_aware_gpt_data_step
	...
	def configure_model(self, tokenizer, pre_process=None, post_process=None) -> "NonStructAwareMCoreGPTModel":
		...
		model = NonStructAwareMCoreGPTModel(
			self,
